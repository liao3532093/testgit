...
<head>
    ...
    <script src="${root}/script/system/prediction_type.js" type="text/javascript"></script>
    <script type="text/javascript">
        $(function () {
            isPermission('#pred_install', '#pred_update', '#prediction_table', 'predic');
        });
        $(document).ready(function () {
            $('#pred_update').attr('disabled', true);
            $('#pred_update').css('background', '#888888');
            //改变每页数据条数,例如改为50
            $("#prediction_table").datagrid('getPager').pagination('refresh', {pageSize: 50});
        });
    </script>
</head>
<body class="easyui-layout">
<div id="pred_top" class="pad-10">
    <form id="prediction_form" method="post">
        <div class="explain-col">
            类型名称:&nbsp;&nbsp;<input id="pred_name" name="title" type="text"/>
            &nbsp;&nbsp;<input id="pred_install" type="button" value="添加" onclick="addPrediction()"/>
            &nbsp;&nbsp;<input id="pred_update" type="button" value="修改" onclick="addPrediction()"/>
            <input id="prediction_id" name="id" type="hidden" value="-1"/>
        </div>
    </form>
</div>
<div region="center" class="pad-10">
    <table id="prediction_table" class="easyui-datagrid"
           url="${root}/prediction/selectPredictionAll.asp"
           pagination="true" fit="true"
           rownumbers="false" fitColumns="true" singleSelect="true" toolbar="#pred_top">
        <thead>
        <tr>
            <th field="id" width="80" align="center">类型ID</th>
            <th field="title" width="200" align="center">预测类型</th>
